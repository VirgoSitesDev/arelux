<script lang="ts">
	...
	onMount(async () => {
		...
		systems = await Promise.all(data.systems.map(async (systemId: string) => {
			...
			const mainImage = supabase.storage.from(TENANT).getPublicUrl(`images/main_${id}.jpg`).data.publicUrl;
			const productImage1 = supabase.storage.from(TENANT).getPublicUrl(`images/product1_${id}.jpg`).data.publicUrl;
			const productImage2 = supabase.storage.from(TENANT).getPublicUrl(`images/product2_${id}.jpg`).data.publicUrl;
			const productImage3 = supabase.storage.from(TENANT).getPublicUrl(`images/product3_${id}.jpg`).data.publicUrl;
    ...
    if (selectedSystem && selectedSystem.id) {
        window.location.href = `/${selectedSystem.id}`;
    }
}
</script>
