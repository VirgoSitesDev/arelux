...
const TENANT = 'arelux-italia'; // Hardcoded tenant

async function loadSettings(supabase: SupabaseClient<Database>): Promise<Settings> {
...
async function loadSystems(supabase: SupabaseClient<Database>): Promise<string[]> {
...
export const load: LayoutLoad = async () => {
	...
	const originalCatalog = await loadCatalog(supabase);

	const [families, systems, joiners, settings] = await Promise.all([
		loadFamilies(supabase, originalCatalog),
		loadSystems(supabase),
		loadJoiners(supabase),
