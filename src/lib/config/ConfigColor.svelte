<!-- src/lib/config/ConfigColor.svelte -->
<script lang="ts">
	...
	let {
		items,
		onsubmit,
		disabled = false, // NUOVO: Aggiunto supporto per disabled
		value = $bindable(),
	}: {
		items: string[];
		onsubmit?: (color: string) => void;
		value?: string;
		disabled?: boolean; // NUOVO
	} = $props();
...
<div class="flex items-center justify-center gap-3 rounded bg-box p-3 {disabled ? 'opacity-50' : ''}">
	{$fnct("config.color")}

	{#if filteredColors().length > 1}
		<Popover.Root bind:open>
			<Popover.Trigger disabled={disabled} class="disabled:cursor-not-allowed">
				<div
					class="h-8 w-8 rounded-[8px] border-2 border-gray-500 {disabled ? 'cursor-not-allowed opacity-50' : ''}"
			...
			>
				{#each filteredColors() as color}
					<button
						type="button"
						class="h-8 w-8 cursor-pointer rounded-[8px] border-2 border-gray-500 disabled:cursor-not-allowed disabled:opacity-50"
	...
	{:else}
		<div
			class="h-8 w-8 cursor-not-allowed rounded-[8px] border-2 border-gray-500 {disabled ? 'opacity-50' : ''}"
			style="background-color: {value};"
		></div>
	{/if}
</div>